...
/**
 * Dedicated Bootstrap Server for AWS EBS Deployment
 * 
 * This is a lightweight, reliable bootstrap server specifically designed
 * for AWS Elastic Beanstalk. It provides:
 * - Peer registration and discovery
 * - Socket.IO WebSocket relay for NAT traversal
 * - Health monitoring
 * - Simple, reliable operation in containerized environments
 * 
 * The unified DIG nodes connect to this for peer discovery.
 */
...
console.log('🚀 Starting Dedicated Bootstrap Server for DIG Network')
...
app.post('/register-turn-server', (req, res) => {
  try {
...
// Store relay endpoint
app.post('/relay-store', (req, res) => {
  try {
    const { storeId, fromPeerId, toPeerId, turnServerId } = req.body
    
    if (!storeId || !fromPeerId || !toPeerId) {
      return res.status(400).json({ error: 'Missing required fields: storeId, fromPeerId, toPeerId' })
    }

    console.log(`🔄 Store relay request: ${storeId} from ${fromPeerId} to ${toPeerId}`)

    // Try to use peer TURN servers first
    if (turnServerId) {
      const turnServer = turnServers.get(turnServerId)
      if (turnServer && turnServer.turnLoad! < turnServer.turnCapacity!) {
        turnServer.turnLoad = (turnServer.turnLoad || 0) + 1
        console.log(`📡 Routing via TURN server: ${turnServerId}`)
    ...
    if (targetSocket) {
      targetSocket.emit('store-request', {
        storeId,
        fromPeerId,
        requestId: `req_${Date.now()}`
      })
      console.log(`🔌 Routing via WebSocket relay to ${toPeerId}`)
      res.json({ success: true, method: 'websocket-relay' })
    } else {
...
httpServer.listen(PORT, '0.0.0.0', () => {
  console.log('✅ Dedicated Bootstrap Server started successfully!')
  ...
  console.log(`   DIG_BOOTSTRAP_NODES="http://[AWS_HOSTNAME]:${PORT}"`)
})
