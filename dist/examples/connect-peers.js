...
async function main() {
    ...
    try {
        ...
        connectionInfo.listeningAddresses.forEach((addr) => {
            console.log(`   ${addr}`);
        });
        ...
        updatedInfo.connectedPeers.forEach((peer) => {
            console.log(`   ${peer}`);
        });
        ...
        for (const [peerId, stores] of Object.entries(finalInfo.peerStores)) {
            console.log(`   ${peerId}: ${stores.length} stores`);
        }
...
//# sourceMappingURL=connect-peers.js.map