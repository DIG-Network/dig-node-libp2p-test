 ...
 */
export function isValidDIGURN(urn) {
 ...
 */
export function isValidDIGURL(url) {
    try {
 ...
 */
export function extractStoreId(identifier) {
 ...
 */
export function extractFilePath(identifier) {
    if (identifier.toLowerCase().startsWith('urn:dig:chia:')) {
        try {
            const nss = identifier.substring(14);
            const slashIndex = nss.indexOf('/');
 ...
 */
export function guessMimeTypeFromPath(filePath) {
