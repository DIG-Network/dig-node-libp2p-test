...
Array < { peerId: string, cryptoIPv6: string, capabilities: NodeCapabilities } > {
    const: turnServers, Array() { peerId: string, cryptoIPv6; string, capabilities; NodeCapabilities; }
} > ;
[];
for (const [peerId, peerInfo] of this.privacyOverlayPeers) {
...
Array < { peerId: string, cryptoIPv6: string, capabilities: NodeCapabilities } > {
    const: bootstrapServers, Array() { peerId: string, cryptoIPv6; string, capabilities; NodeCapabilities; }
} > ;
[];
for (const [peerId, peerInfo] of this.privacyOverlayPeers) {
...
async;
pullPeersFromBootstrap();
Promise < void  > {
    : .config.discoveryServers || this.config.discoveryServers.length === 0
};
{
...
inferCapabilitiesFromPeer(peer, any);
NodeCapabilities;
{
    return {
        libp2p: true,
...
async;
downloadStoreWithFullFallback(storeId, string);
Promise < boolean > {
...
async;
downloadStoreFromDHT(storeId, string);
Promise < boolean > {
    try: {
        const: dht = this.node.services.dht,
        if(, dht) {
...
private;
async;
downloadStoreFromGossipNetwork(storeId, string);
Promise < boolean > {
    try: {
        : .privacyOverlayPeers
    }
...
async;
downloadStoreFromDistributedTurn(storeId, string);
Promise < boolean > {
    try: {
...
async;
downloadStoreViaMeshRouting(storeId, string);
Promise < boolean > {
    try: {
        const: connectedPeers = this.node.getPeers(),
...
async;
downloadStoreViaBootstrapTurn(storeId, string);
Promise < boolean > {
    try: {
        const: bootstrapUrl = this.config.discoveryServers?.[0],
        if(, bootstrapUrl) {
...
async;
downloadStoreFromSpecificPeer(storeId, string, peerId, string, cryptoIPv6, string);
Promise < boolean > {
    try: {
...
async;
queryPeerForStore(peerId, string, storeId, string);
Promise < { peerId: string, cryptoIPv6: string } | null > {
    try: {
        const: peer = this.node.getPeers().find(p => p.toString() === peerId),
        if(, peer) {
            return null;
...
return null;
async;
downloadStoreViaPeerTurn(storeId, string, sourcePeer, any, turnServer, any);
Promise < boolean > {
    try: {
...
async;
downloadStoreViaRoute(storeId, string, route, any[]);
Promise < boolean > {
    try: {
        if(route) { }, : .length === 0
    }
...
async;
requestStoreRoute(peerId, string, storeId, string);
Promise < any[] > {
    try: {
...
isSelfRelay(bootstrapUrl, string, ourPort, number);
boolean;
{
    try {
...
async;
handleProtocolHandshake(request, any, peerId, string);
Promise < DIGResponse > {
    try: {
        const: {
            networkId,
        ...
        if(peerCapabilities) { }
    } && Array.isArray(peerCapabilities)
};
{
...
async;
initiateProtocolHandshake(peerId, string);
Promise < void  > {
    try: {
        const: peer = this.node.getPeers().find(p => p.toString() === peerId),
        if(, peer) { }, return: ,
        ...
        const: self = this,
        await, pipe([uint8ArrayFromString]) { }
...
async;
handleDIGRequest({ stream }, { stream: Stream });
Promise < void  > {
    const: self = this,
...
async * serveFileFromURN(urn, string);
...
async * serveFileRange(storeId, string, filePath, string, rangeStart, number, rangeEnd, number, chunkId ?  : string);
AsyncGenerator < Uint8Array > {
    ...
    try: {
        ...
        // Validate range
        if(rangeStart, , ) { }
    } || rangeEnd >= totalSize || rangeStart > rangeEnd
};
{
    const response = {
        success: false,
...
async * serveFileFromStore(storeId, string, filePath, string);
AsyncGenerator < Uint8Array > {
    const: digFile = this.digFiles.get(storeId),
    if(, digFile) {
        const response = {
...
async;
handleDiscoveryRequest({ stream }, { stream: Stream });
Promise < void  > {
    const: self = this,
    try: {
        await, pipe(stream, async ) { }, function:  * (source), any
    }
};
{
    for await (const chunk of source) {
...
private;
async;
handleFileSystemEvent(event, any);
Promise < void  > {
...
async;
addOrUpdateStore(filePath, string, storeId, string);
Promise < void  > {
...
async;
discoverPeerStores(peerId, string);
Promise < void  > {
    try: {
        const: peer = this.node.getPeers().find(p => p.toString() === peerId),
        if(, peer) { }, return: 
        ...
        ,
        : .peerProtocolVersions.has(peerId)
    }
};
{
...
async;
downloadStoreFromPeers(storeId, string);
Promise < void  > {
...
async;
downloadStoreInParallelFromPeers(storeId, string, availablePeers, (Array));
Promise < boolean > {
    try: {
...
async;
downloadChunkFromLibP2PPeer(peerId, string, peer, any, storeId, string, rangeStart, number, rangeEnd, number, chunkIndex, number, chunks, (Map));
Promise < boolean > {
    try: {
...
async;
executeConcurrentDownloads(tasks, Promise < boolean > [], maxConcurrency, number);
Promise < boolean[] > {
...
async;
collectChunkResponse(stream, Stream, expectedChunkId, string);
Promise < Buffer | null > {
    try: {
...
private;
assembleChunks(chunks, (Map), totalChunks, number);
Buffer | null;
{
    try {
        const orderedChunks = [];
...
async;
getFileSizeFromPeer(peerId, string, storeId, string);
Promise < number | null > {
    try: {
...
async;
saveDownloadedStore(storeId, string, content, Buffer);
Promise < void  > {
    try: {
        const: filePath = join(this.digPath, `${storeId}.dig`),
        await,
...
async;
downloadStoreFromLibP2PPeer(storeId, string, peerId, string, peer, any);
Promise < void  > {
...
async;
downloadStoreViaTurnServers(storeId, string);
Promise < void  > {
    const: bootstrapUrl = this.config.discoveryServers?.[0],
    if(, bootstrapUrl) {
        throw new Error('No bootstrap server configured');
...
Promise < void  > {
    ...
    ,
    // Try to connect to new peers
    for(, address, of, knownAddresses) { }, : .slice(0, 5)
};
{ // Reduce to 5 attempts per round
    try {
...
Promise < void  > {
    : .config.connectToPeers || this.config.connectToPeers.length === 0
};
{
    return;
}
...
await this.connectToDiscoveredPeers();
async;
connectViaRelay(targetPeerId, string);
Promise < any > {
    : .webSocketRelay || !this.webSocketRelay.isConnected()
};
{
...
async;
initiateRelayConnection(targetPeerId, string);
Promise < any > {
...
async;
discoverPeerStoresViaRelay(peerId, string);
Promise < void  > {
    try: {
...
async;
downloadStoreViaRelay(peerId, string, storeId, string);
Promise < void  > {
    try: {
...
async;
downloadStoreViaBootstrap(storeId, string);
Promise < void  > {
    try: {
